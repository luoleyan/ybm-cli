...
async function installToolDependencies(targetDir, packageManager, dependencies, devDependencies) {
  ...
  try {
    ...
    // 尝试使用国内镜像
    const useRegistry = '--registry=https://registry.npmmirror.com';

    // 安装依赖
    if (dependencies.length > 0) {
      const depsString = dependencies.join(' ');
      let installCommand = packageManager === 'yarn' ? `yarn add ${depsString}` :
                         packageManager === 'pnpm' ? `pnpm add ${depsString}` : `npm install ${depsString} --save`;

      // 为npm添加镜像
      if (packageManager === 'npm') {
        installCommand += ` ${useRegistry}`;
      }
      ...
      while (!success && retries < maxRetries) {
        try {
          ...
          spinner.text = `安装依赖失败，正在重试 (${retries}/${maxRetries}): ${depsString}`;
          // 等待一段时间再重试
    ...
    if (devDependencies.length > 0) {
      ...
      let installDevCommand = packageManager === 'yarn' ? `yarn add ${devDepsString} --dev` :
                            packageManager === 'pnpm' ? `pnpm add ${devDepsString} -D` : `npm install ${devDepsString} --save-dev`;

      // 为npm添加镜像
      if (packageManager === 'npm') {
        installDevCommand += ` ${useRegistry}`;
      }
      ...
      while (!success && retries < maxRetries) {
        try {
          ...
          spinner.text = `安装开发依赖失败，正在重试 (${retries}/${maxRetries}): ${devDepsString}`;
          // 等待一段时间再重试
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
      }
