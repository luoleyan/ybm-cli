...
function getUIFrameworkChoices(template) {
  // 处理TypeScript模板名称，提取基本模板名称
  let baseTemplate = template;

  // 如果是TypeScript模板，移除-ts后缀
  if (template.endsWith('-ts')) {
    baseTemplate = template.replace(/-ts$/, '');
  }

  // 尝试使用基本模板名称查找UI框架选项
  if (toolsConfig.uiFramework.choices[baseTemplate]) {
    return toolsConfig.uiFramework.choices[baseTemplate];
  }
...
function getTypeScriptPackages(template) {
  ...
  // 尝试使用基本模板名称查找TypeScript包
  if (toolsConfig.typescript.packages[baseTemplate]) {
    return toolsConfig.typescript.packages[baseTemplate];
  }
...
function getTestingPackages(template, testingFramework) {
  ...
  // 如果没有找到测试框架，返回空数组
  if (!framework || !framework.packages) {
    return [];
  }

  // 尝试使用基本模板名称查找测试包
  if (framework.packages[baseTemplate]) {
    return framework.packages[baseTemplate];
  }
...
function getCSSPreprocessorPackages(template, preprocessor) {
  ...
  // 如果没有找到CSS预处理器，返回空数组
  if (!processor || !processor.packages) {
    return [];
  }

  // 尝试使用基本模板名称查找CSS预处理器包
  if (processor.packages[baseTemplate]) {
    return processor.packages[baseTemplate];
  }
